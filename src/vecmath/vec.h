...
public:
	...
	explicit Vec3( int ) { n[0] = 0.0; n[1] = 0.0; n[2] = 0.0; }
	Vec3( const Vec4<T>& v )
		{ n[0] = v[0]; n[1] = v[1]; n[2] = v[2]; }
	...
	// Vec3<T> X(const Vec3<T>& a) const {return Vec3<T>(n[1]*a[2] - n[2]*a[1], -n[0]*a[2] + n[2]*a[0], n[0]*a[1] - n[1]*a[0] );}
	//---[ Conversion Operators ]----------------
	...
	template <class U> friend Vec3<T> power( const Vec3<T>& a, const double d );
};
...
template <class T> 
inline Vec3<T> power( const Vec3<T>& a, const double d ){
	return Vec3<T>( pow(a[0],d), pow(a[1],d), pow(a[2],d) );
}

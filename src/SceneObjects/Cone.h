...
public:
    ...
    virtual BoundingBox ComputeLocalBoundingBox()
    {
		...
		localbounds.setMin(Vec3d(-biggest_radius, -biggest_radius, (height < 0.0f)?(height):(0.0f)));
		localbounds.setMax(Vec3d(biggest_radius, biggest_radius, (height < 0.0f)?(0.0f):(height)));
        return localbounds;
    }
